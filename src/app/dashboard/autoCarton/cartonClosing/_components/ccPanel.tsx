...
const CCPanel: React.FC<CCPanelProps> = ({ accentColor, name, faults,  onClickLink }) => {
	...
	const TotalFaults = faults.reduce((count: number, fault: any) => {
		return fault.faultString !== "box" && fault.faultString !== "Watchdog timer expired" ? count + 1 : count;
	}, 0);
...
let faultsHigher = false;
    ...
    return (
        ...
        <div className="flex gap-x-5" >
            <div>
                <div
                    className={`h-32 w-32 flex items-center justify-center rounded-full text-white bg-gray-500`}
                >
                    <div
            ...
            <div className="flex flex-col">
                ...
                <div className="flex">
                    ...
                    <div className="ml-4 mt-0.5 h-5 w-5 rounded-full bg-gray-500">
                        <div className={`ml-0.5 mt-0.5 h-4 w-4 rounded-full ${connectionStatusConnected ? 'bg-green-500' : 'bg-red-500'}`}></div>
                    </div>
                </div>
            </div>
        </div>
